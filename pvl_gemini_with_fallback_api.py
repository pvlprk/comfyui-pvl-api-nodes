...
# Single-image / text multimodal node.
...
def _openai_post(url: str, payload: Dict[str, Any], api_key: str, timeout: int, debug: bool) -> requests.Response:
    ...
    try:
        ...
        if debug:
            print(f"[PVL_GEMINI] OpenAI request timeout after {timeout}s")
        class TimeoutResponse:
            status_code = 599
            text = "Client timeout"
        ...
        if debug:
            print(f"[PVL_GEMINI] OpenAI request exception: {e}")
        raise
            ...
            for fut in as_completed(futs):
                ...
                if ok:
                    ...
                    if "timeout" in lower or "429" in lower or "503" in lower:
                        ...
                        print(f"[PVL_GEMINI] (OpenAI {label} batch item {idx+1}) API error: {payload}", flush=True)

        pending = next_round
        if pending and attempt < max_tries:
            delay = attempt  # linear backoff
            if debug:
                print(f"[PVL_GEMINI] [OpenAI {label}] Waiting {delay}s before retry...")
            time.sleep(delay)
...
# Node
# -----------------------------
class PVL_Gemini_with_fallback_API:
    @classmethod
    def INPUT_TYPES(cls):
    ...
    def run(self, model: str, thinking_level: str, tries: int, timeout: int, temperature: float,
        ...
        if force_openai:
            ...
            pil_images = [pil_img] if pil_img is not None else []
            ...
            print(f"[PVL_GEMINI] Completed via OpenAI only in {elapsed:.2f}s (batch={batch}, tries={tries})", flush=True)
        ...
        while pending and attempt < tries:
                ...
                for fut in as_completed(futs):
                    ...
                    if ok:
                        ...
                        if "prompt blocked" in lower or "blockreason" in lower:
                            ...
                            print(f"[PVL_GEMINI] (batch item {idx+1}) blocked by Gemini: {payload}", flush=True)
            ...
            if pending and attempt < tries:
                delay = attempt  # linear backoff
                if debug:
                    print(f"[PVL_GEMINI] Waiting {delay}s before retry...")
                time.sleep(delay)
...
NODE_CLASS_MAPPINGS = {"PVL_Gemini_with_fallback_API": PVL_Gemini_with_fallback_API}
NODE_DISPLAY_NAME_MAPPINGS = {"PVL_Gemini_with_fallback_API": "PVL Gemini with fallback Api"}
