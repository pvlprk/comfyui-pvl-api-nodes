...
def _openai_build_input(prompt: Optional[str], pil_img: Optional[Image.Image]) -> List[Dict[str, Any]]:
    content: List[Dict[str, Any]] = []
    if prompt and str(prompt).strip():
        content.append({"type": "input_text", "text": str(prompt)})
    if pil_img is not None:
        # Use data URL form for multimodal input; mask base64 in debug logs elsewhere.
        data_b64 = _b64_from_pil(pil_img, mime="image/png")
...
def _openai_post(url: str, payload: Dict[str, Any], api_key: str, timeout: int, debug: bool) -> requests.Response:
    ...
    try:
        ...
        if debug:
            print(f"[PVL_GEMINI_SPECIAL] OpenAI request timeout after {timeout}s")
        class TimeoutResponse:
            status_code = 599
            text = "Client timeout"
        ...
        if debug:
            print(f"[PVL_GEMINI_SPECIAL] OpenAI request exception: {e}")
        raise
...
def _openai_generate_once(
    ...
    input_block = _openai_build_input(prompt, pil_img)
...
class PVL_Gemini_API_avatar_special:
    ...
    Optional OpenAI fallback / override for failed Gemini calls.
    """
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            ...
            "optional": {
                ...
                "openai_model": (OPENAI_MODEL_CHOICES, {"default": "GPT-5.1"}),
    ...
    def _per_call_prompt_factory(self, header: str, section_delimiter: str, tail_parts: List[str],
                                 append_variation_tag: bool, batch: int):
        """
        Returns a function per_call_prompt(i) that builds a full prompt for given 1-based index.
        """
                ...
                for fut in as_completed(futs):
                    ...
                    if ok:
                        ...
                        if "timeout" in lower or "429" in lower or "503" in lower:
                            ...
                            print(f"[PVL_GEMINI_SPECIAL] (OpenAI {label} batch item {idx+1}) API error: {payload}", flush=True)

            pending = next_round
            if pending and attempt < max_tries:
                delay = attempt  # linear backoff
                if debug:
                    print(f"[PVL_GEMINI_SPECIAL] [OpenAI {label}] Waiting {delay}s before retry...")
                time.sleep(delay)
    ...
    def run(self, model: str, tries: int, timeout: int, temperature: float,
            ...
            openai_model: str = "GPT-5.1", openai_api_key: str = ""):

        start_time = time.time()

        # Prepare shared prompt components first.
        ...
        per_call_prompt = self._per_call_prompt_factory(header, section_delimiter, tail_parts, append_variation_tag, batch)

        # If force_openai is ON, skip Gemini entirely and only call OpenAI.
        if force_openai:
            ...
            oa_results, oa_errors, oa_pending = self._run_openai_for_indices(
                indices=indices,
            ...
            # If any indices still have no successful result, raise a clear error.
            ...
            print(f"[PVL_GEMINI_SPECIAL] Completed via OpenAI only in {elapsed:.2f}s (batch={batch}, tries={tries})", flush=True)
            return (combined_force,)

        # Otherwise: primary Gemini path (existing behaviour), possibly with OpenAI fallback.
        ...
        # Determine which indices, if any, still have no successful result.
        failed_indices = [i for i in range(batch) if i not in results]

        # If there are failures and OpenAI fallback is enabled, send ONLY those to OpenAI.
        if failed_indices and openai_fallback:
            openai_key = _get_openai_api_key(openai_api_key)
            if not openai_key:
                # If we can't fallback, behave like original node and raise on the first failed index.
                first_idx = failed_indices[0]
                msg = last_errors.get(first_idx, "Unknown error")
                failed_str = ", ".join(str(i + 1) for i in failed_indices)
                raise RuntimeError(f"Gemini API failed for batch item(s) {failed_str}: {msg} (and no OpenAI key available for fallback)")

            remaining_tries = max(1, tries - attempt)
            if debug:
                _log_debug(
                    debug,
                    f"Using OpenAI fallback for indices {[i+1 for i in failed_indices]} with remaining_tries={remaining_tries}, model={openai_model}"
                )
        ...
        # If we still have failures and no fallback, raise as before.
