...
class PVL_Switch_x15:
    ...
    def check_lazy_status(self, switch_condition, **kwargs):
        ...
        ComfyUI passes inputs as keyword arguments, so we read from kwargs.
        """
        lazy_inputs = []

        # Collect case and input values from kwargs
        case_values = [kwargs.get(f"case_{i}") for i in range(1, 16)]  # case_1 ... case_15
        input_default = kwargs.get("input_default")
        input_values = [kwargs.get(f"input_{i}") for i in range(1, 16)]  # input_1 ... input_15

        # Case-specific lazy resolution
        for idx, case_value in enumerate(case_values, start=1):
            if switch_condition == case_value:
                # If the chosen input is None (not yet evaluated), mark it as lazy
    ...
    def switch_case(self, switch_condition, **kwargs):
        """
        Main switch logic. Also uses kwargs instead of positional args.
        """
